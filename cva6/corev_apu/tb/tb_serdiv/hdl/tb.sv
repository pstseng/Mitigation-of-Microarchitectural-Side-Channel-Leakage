...
// latency = (first out_vld_o) - (handshake in_vld_i && in_rdy_o)
// ------------------------------------------------------------
  longint unsigned cyc;

  longint unsigned lat_sum;
  longint unsigned lat_cnt;
  longint unsigned lat_min;
  longint unsigned lat_max;
  longint unsigned start_t, end_t;
  ...
  initial begin : p_acq
    ...
    repeat(1) begin
      ...
      do begin
        ...
        // compute cycle
        longint unsigned start_cyc, end_cyc;
        ...
        // update latency stats (count every completed vector)
        lat_cnt++;
        lat_sum += lat;
        ...
        if(VERBOSE | !ok) begin
            ...
            if (!ok) tmpstr = $sformatf("vector: %04d> %0d %s %0d = %0d != %0d -> error!!", acqCnt, op_a_tmp, opStr, op_b_tmp, res, act);
            else tmpstr = $sformatf("vector: %04d> %0d %s %0d = %0d == %0d ", acqCnt, op_a_tmp, opStr, op_b_tmp, res, act);

            // $display("TB> %s", tmpstr);
            $display("TB> %s [Latency: %0d cycles]", tmpstr, lat);
        end

        if(!ok) begin
          // failingTests = $psprintf("%sTB> %s\n", failingTests, tmpstr);
          failingTests = $sformatf("%sTB> %s\n", failingTests, tmpstr);
    ...
    if (lat_cnt != 0) begin
      real lat_avg;
      lat_avg = real'(lat_sum) / real'(lat_cnt);
      $display("TB> ----------------------------------------------------------------------");
      $display("TB> LATENCY SUMMARY (cycles): count=%0d  min=%0d  max=%0d  avg=%0f",
               lat_cnt, lat_min, lat_max, lat_avg);
      $display("TB> ----------------------------------------------------------------------\n");
    end
...
//     forever begin
//         @(posedge clk_i);
//         // 當狀態機在 DIVIDE 狀態時，印出目前的計數器狀態
//         // 假設你的內部計數器信號叫做 cnt_q
//         if (i_dut.state_q == 2'd1) begin
//             $display("      [Cycle %0d] 正在處理第 %0d 位元...", cyc, i_dut.cnt_q);
//         end
//     end
// end
